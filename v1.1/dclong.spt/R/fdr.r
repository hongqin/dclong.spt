...
#'
#' @param cutoff a vector of cut-offs. For a given cut-off, the null 
...
#' To specify an 
#' appropriate error tolerance is important if there are lots of 
#' pvalues that are very close to a cut-off. 
#' An extreme but happen-often (e.g. when calculating qvalues) case
#' is that a cut-off is inside the support of sequential pvalues. 
#' A value no greater than \code{h/(100*n^2)} is recommended. 
#' Please refer to \code{\link{spt.corr}} for documentation of \code{h}
#' and {n}.
#' @export
